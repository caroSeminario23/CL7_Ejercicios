...
fun main() {
    ...
    val eventosAgrupados = eventos.groupBy { it.parteDia }
    val eventosMañana = eventosAgrupados[ParteDia.MAÑANA] ?: emptyList()
    val eventosTarde = eventosAgrupados[ParteDia.TARDE] ?: emptyList()
    val eventosNoche = eventosAgrupados[ParteDia.NOCHE] ?: emptyList()

    println("DETALLE DE LOS EVENTOS POR PARTE DEL DÍA:")
    println("-----------------------------------------")
    eventosAgrupados.forEach { (parteDia, eventos) ->
        println("$parteDia: ${eventos.size} eventos")
    }
}